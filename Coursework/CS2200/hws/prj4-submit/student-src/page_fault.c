...
uint64_t page_fault_handler(uint64_t vpn, char rw, stats_t *stats)
{
	...
	(current_pagetable + vpn)->frequency = 1;
	if (rw == 'r') {
...
static uint64_t find_free_frame(stats_t *stats)
{
	...
	/********* TODO ************/
	for (int i = 0; i < (1<<rlt_size); i++) {
		if (!((rlt + i)->valid)) {
			return i;
		}
	}

	uint64_t victim_frequency = (rlt[0].task_struct->pagetable[rlt[0].vpn]).frequency;

	for (int i = 0; i < (1<<rlt_size); i++) {
		if ((rlt[i].task_struct->pagetable[rlt[i].vpn]).frequency < victim_frequency) {
			victim_frequency = (rlt[i].task_struct->pagetable[rlt[i].vpn]).frequency;
			victim_pfn = i;
		}
	}
